...
mod tests {
    ...
    async fn virtual_render_fallback_cursor_when_viewport_none_and_reveal_hidden_for_cjk_ime() {
        ...
        assert!(
            ...
            "reveal_hidden_cursor_for_cjk_ime should ensure a visible cursor even when the pane hides it; got {cursor:?}",
        );
    }

    #[tokio::test]
    async fn virtual_render_skips_reveal_when_agent_filter_does_not_match() {
        let mut state = AppState::test_new();
        state.reveal_hidden_cursor_for_cjk_ime = true;
